...
function updateBar(element) {
    element.select('rect').transition().duration(1000)
        .attr('width', d => d)
        .attr('fill', colors['update'])

    element.select('text').text(d => d)
}

function draw() {
    fillArray(data)
    
    let elements = elementGroup.selectAll('.element').data(data);
    
    elements.enter().append('g').attr('class', 'element').call(drawBar)
